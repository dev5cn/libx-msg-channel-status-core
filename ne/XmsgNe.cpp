...
void XmsgNe::estab()
{
	...
	req->set_neg(X_MSG_CHANNEL_STATUS);
	req->set_cgt(XmsgChannelStatusCfg::instance()->cfgPb->cgt());
	req->set_salt(Crypto::gen0aAkey256());
	req->set_sign(Crypto::sha256ToHexStrLowerCase(XmsgChannelStatusCfg::instance()->cfgPb->cgt() + req->salt() + this->pwd));
