...
XscMsgItcpRetType XmsgAp::itcp(XscWorker* wk, XscChannel* channel, shared_ptr<XscProtoPdu> pdu)
{
	...
	if (stub == nullptr)
	{
		XmsgImLog::getLog(channel)->transPassStart(trans, pdu);
	...
	if (!ucgt->isSameHlr(XmsgChannelStatusCfg::instance()->uhlr)) 
	{
	...
	shared_ptr<XmsgClient> client(new XmsgClient(cgt, plat, did, ccid));
	trans->beginMsg = req;
	((void (*)(shared_ptr<XmsgNeUsr> nu, const string& ap, shared_ptr<XmsgClient> client, SptrXitp trans, shared_ptr<Message> req)) (stub->cb))(ne, ne->uid, client, trans, req);
